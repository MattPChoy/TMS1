...
/**
 * From JavaDoc (https://csse2002.uqcloud.net/assignment/1/tms/intersection/Intersection.html)
 * public class Intersection extends Object

 * Represents a point at which routes can originate and terminate
 * All intersections have a unique identifier (ID), a list of incoming connections and, optionally, a set of traffic
   lights.
...
public class Intersection {
     ...
     *                     Do not need to check if the id is an empty string / null
     *
     ...
     * Adding/removing routes from this list should not affect the intersection's internal list of connecting
     routes.
    ...
    public List<Intersection> getConnectedIntersections(){
        // Note that connections could be generated when this method is called, by iterating through the incomingRoutes
        // list and appending route.from() to a list and returning that list.

        return new ArrayList<>(this.connections); // Return a copy so that the original list object is not edited when
                                                  // operations are performed to this list.
    }

    /**
     * Creates a new Route originating from the given intersection and adds it to our list of incoming routes.
     * The ID of the new route should be of the form "from:to" where 'from' is the ID of the origin intersection and
       'to' is the ID of this intersection. The new route should have a default speed given by 'defaultSpeed', however
       if this value is negative, then an exception should be thrown and no new connection should be added.

     * If this intersection has an IntersectionLights a new traffic light signal should be added to the new route.

     * If there is already a connection from 'from', then instead of creating a new connection, an IllegalStateException
       should be thrown and the method should do nothing.

     * Note: On Piazza (@124): Do not need to check if a route has been created circularly (back to itself)
    ...
    public void addConnection(Intersection from, int defaultSpeed) throws IllegalArgumentException,
        ...
        // If the code below this runs, the 'illegal' cases have been taken care of.
        // A route goes from -- ROUTE --> this
        String RouteID = from.getId() + ":" + this.getId();
        Route newRoute = new Route(RouteID, from, defaultSpeed);
        incomingRoutes.add(newRoute);
     ...
     * The format of the string to return is simply "id" where 'id' is this intersection's identifier string.

     * For example, an intersection with the an ID of "ABC" and traffic lights with a string representation of "3:X,Y,Z"
     * would have a toString() value of "ABC:3:X,Y,Z".
